...
def analyse_input(file_name):
    ...
    while pair:
        if pair == '\n':
            pass
        elif pair[0:11] == 'PERIODICITY':
            ret_n.append(int(pair.split('\t')[-1]))
        elif pair[0:7] == 'suction':
            ret_z = ret_z + 1
            flag=0
        elif pair[0:8]=='pressure':
            flag=1
        elif pair[0:9] == 'SECTIONAL':
            pair = f.readline()
            # ret_zr.append(int(pair))
            ret_zr = int(pair)
        elif pair[0] == '#':
            pair = f.readline()  # XYZ
            tot = int(f.readline())  # 数字
            if flag == 0:  # suction
                ret_p.append([])
            else:
                ret_s.append([])
            for i in range(tot):
                pair = f.readline().split(' ')#这里不一定用‘ ’分割，可能用\t
                if flag == 0:  # suction
                    ret_p[-1].append([float(pair[0]), float(pair[1]), float(pair[2])])
                else:
                    ret_s[-1].append([float(pair[0]), float(pair[1]), float(pair[2])])
        pair = f.readline()
        # print(np.shape(ret_p))
        # print(ret_s)
    # print(len(ret_p))
    # print(len(ret_p[0]))
    # print(len(ret_p[0][0]))
    return int(ret_z / 2), int(ret_zr), ret_n, ret_p, ret_s
...
def print_NIROW_info(number_of_blades, file):
    ...
    geom_print(f'PERIODICITY\t{number_of_blades}\n', file)
