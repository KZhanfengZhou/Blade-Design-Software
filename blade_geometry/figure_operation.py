...
def get_origin_control_point(num, opt, init_cp_dis=None, te_radius=None,
    ...
    if opt is False:
        if init_cp_dis is None:
            init_cp_dis = np.array(
                [0.55265057, 1.45065745, 1.9049298, 1.4, 1.2, 0.90225495, 0.5696306, 0.12181866,
                 0.03256453384853942])
        if len(init_cp_dis) < num + 3:
            ll = len(init_cp_dis)
            for i in range(num + 3 - ll):
                init_cp_dis = np.append(init_cp_dis, 0.032)
        ...
        if len(init_cp_dis) < num + 3:
            ll = len(init_cp_dis)
            for i in range(num + 3 - ll):
                init_cp_dis = np.append(init_cp_dis, 0.028)
        ret, k, point, last_k_o = Slope.rd_point(camber, init_cp_dis, opt, te_radius, num, te_theta)
...
def controlpoint2line(opt: bool):  # False 改 pp ，True 改 ps
    ...
    if opt is False:
        if curve_type == 1:
            re_p = calcu.blade_curve(pp, npoint - 1)
        else:
            re_p = calcu.blade_curve(pp, npoint - 1, pp_node_vector)
        ...
        if curve_type == 1:
            re_s = calcu.blade_curve(ps, npoint - 1)
        else:
            re_s = calcu.blade_curve(ps, npoint - 1, ps_node_vector)
...
def auto_plot(fig, ax, init_pp=None, init_ps=None):  # , fig3d, ax3d
    ...
    meanline = Bias_line.meanline(re_p, re_s)
