...
from blade_geometry import b_spline
...
def blade_curve(ctrl_point, M, nodeVector=None):  # 有nodeVector就是B样条
    # if nodeVector is None:
    #     return bezier_curve(ctrl_point, M)
    # else:
    #     return b_spline.draw_b_spline(ctrl_point, M, nodeVector)

    if len(ctrl_point) <= 3:  # 弧度曲线 轮毂曲线强制用贝塞尔
        return bezier_curve(ctrl_point, M)
    else:  # 默认使用均匀b样条，两端重复k+1次，保证为clamped b spline
        if nodeVector is None:
            return bezier_curve(ctrl_point, M)
        else:
            return b_spline.draw_b_spline(ctrl_point, M, nodeVector)
...
def offset_meanline(re_p, re_s):
    offset_line_p = copy.deepcopy(re_p)
    offset_line_s = copy.deepcopy(re_s)
    ret = []
    x = []
    y = []
    for i in range(120):
...
'''
1 偏置步长、步数的选取，怎么保证在两曲线接近时不跳过点，在两图线较远时能完整偏置
2 结果点排序怎么做，单纯按照x排序肯定有问题，可以考虑按照计算顺序来排序
3 叶盆曲线不是凸曲线，怎么确定偏置的方向？
3 怎么加速，现在速度太慢了
'''
...
def offset(pre_Line, delta):  # 求解曲线的偏置线，输入Line，和delta,分别代表曲线和偏置距离，Line输入格式为两列数据，代表x值和y值
    # global flag
    # if flag < 10:
    #     plt.plot(pre_Line[:, 0], pre_Line[:, 1])
    # Line = copy.deepcopy(pre_Line)
    # print(math.fabs(delta))
    Line = pre_Line
    for i in range(len(Line)):
        n_vec = get_side_vector(Line, i, False)  # if delta < 0 else True
        Line[i, 0] = Line[i, 0] + math.fabs(delta) * n_vec[0]
        Line[i, 1] = Line[i, 1] + math.fabs(delta) * n_vec[1]
    # if flag < 10:
...
def get_side_vector(array, i, opt):  # 凹侧好像有问题，偏置线出错
    # 求方向正确的单位法向量
