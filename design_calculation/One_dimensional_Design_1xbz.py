    ...
    if data_list.blade_layout_method == 0:
        ...
        for i in range(Z):
            if i == 0:
                ...
                data_list.Tt0M[i] = data_list.Tt2M[i - 1]  # 级进口总温 asdfg
                data_list.pt0M[i] = data_list.pt2M[i - 1]  # 级进口总压 asdfg
                data_list.hsM[i] = h[i] + data_list.c2M[i - 1] ** 2 / 2000
            # 第一级静叶
            data_list.Dm1M[i] = Dmz  # 静叶平均直径 asf asdfg
            data_list.alph0M[i] = 90  # 改V4，静叶进口气流角 asdfg
            data_list.alph1M[i] = 27.5  # 静叶出口气流角 asdfg
            ...
            data_list.Lamda1M[i] = data_list.c1M[i] / data_list.c_sound_mixM[i]  # 静叶出口折合速度（静叶出口马赫数）asdfg
            data_list.T1_mixM[i] = data_list.Tt1_mixM[i] * (
                    1 - (data_list.k - 1) / (data_list.k + 1) * data_list.Lamda1M[i] ** 2)  # 静叶出口静温(动叶进口) asdfg
            data_list.p1M[i] = data_list.pt0M[i] * (
                    1 - data_list.h1sM[i] * 1000 / (
                    data_list.k / (data_list.k - 1) * data_list.R * data_list.Tt0M[i])) ** (
                                       data_list.k / (data_list.k - 1))  # 静叶出口静压(动叶进口) asdfg
            ...
            data_list.l1M[i] = data_list.A1M[i] / (
                    np.pi * data_list.Dm1M[i] * np.sin(data_list.alph1M[i] * np.pi / 180))  # 静叶出口叶高 asdfg

            # 第一级动叶
            data_list.w1uM[i] = data_list.c1uM[i] - data_list.um1M[i]  # 动叶进口相对圆周分速
            data_list.w1zM[i] = data_list.c1zM[i]  # 动叶进口轴向分速
            data_list.Beta1M[i] = np.arctan(data_list.w1zM[i] / data_list.w1uM[i]) * 180 / np.pi  # 动叶进口相对气流角 asdfg
            # 动叶冷却空气参数
            ...
            # 动叶参数
            data_list.w1M[i] = data_list.w1zM[i] / np.sin(data_list.Beta1M[i] * np.pi / 180)  # 动叶进口相对速度 asdfg
            ...
            data_list.w2sM[i] = np.sqrt(
                ...
                data_list.um2M[
                    i] ** 2)  # 动叶出口理想相对速度
            data_list.R_Tt2M[i] = data_list.Tt11_mixM[i] - np.abs(data_list.um1M[i] ** 2 - data_list.um2M[i] ** 2) / (
                    2 * cp)  # 改改，后面变成了绝对值，动叶出口相对总温
            data_list.w2M[i] = data_list.yRM[i] * data_list.w2sM[i] * (1 + data_list.Grc2M[i] * data_list.wrc2M[i]) / (
                    1 + data_list.Grc2M[i])  # 动叶出口相对速度 asdfg
            ...
            data_list.Lamda12M[i] = data_list.w2M[i] / data_list.c2_sound_mixM[i]  # 动叶出口折合速度（动叶出口相对马赫数） asdfg
            ...
            # 动叶出口气流角计算
            if i == Z - 1:
                data_list.alph12M[i] = data_list.alph2z  # 动叶出口绝对气流角 asdfg
        ...
        for i in range(Z):
            ...
            while True:  # 改
                while np.abs(l1M1 - data_list.l1M[i]) > 0.00001:  # 改
                    ...
                    data_list.c_sound_mixM[i] = np.sqrt(
                        2 * data_list.k / (data_list.k + 1) * data_list.R * data_list.Tt1_mixM[i])  # 静叶出口混合声速
                    data_list.Lamda1M[i] = data_list.c1M[i] / data_list.c_sound_mixM[i]  # 静叶出口折合速度（静叶出口马赫数）
                    data_list.T1_mixM[i] = data_list.Tt1_mixM[i] * (
                ...
                while np.abs(l2M1 - data_list.l2M[i]) > 0.00001:
                    ...
                    data_list.l2M[i] = data_list.G2M[i] / (
                            ...
                            * np.sin(data_list.Beta2M[i] * np.pi / 180))  # 动叶出口叶高 asf asdfg
                    data_list.A2M[i] = np.pi * data_list.Dm2M[i] * data_list.l2M[i]  # 动叶出口截面积
            ...
            data_list.H_to_W1M[i] = 16.4 / (data_list.Dm1M[i] / data_list.l1M[i] - 0.25)  # 静叶叶高叶宽比
            data_list.H_to_W2M[i] = 24 / (data_list.Dm2M[i] / data_list.l2M[i])  # 动叶叶高叶宽比
            data_list.Bz1M[i] = data_list.l1M[i] / data_list.H_to_W1M[i]  # 静叶中径处叶宽 asdfg
            data_list.Bz2M[i] = data_list.l2M[i] / data_list.H_to_W2M[i]  # 动叶中径处叶宽 asdfg
            data_list.ACM[i] = 0.2 * (data_list.Bz1M[i] + data_list.Bz2M[i])  # 轴向间隙 asdfg
            data_list.LM[i] = 1.2 * (data_list.Bz1M[i] + data_list.Bz2M[i])  # 通流部分长度,所有级通流部分长度的加和，记得带上级与级之间的轴向间隙 asdfg
