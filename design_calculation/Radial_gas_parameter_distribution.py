...
def radial_gas_parameter_distribution(blade_stage):  # blade_stage 叶片级数
    ...
    # 通流部分进出口气体参数
    pt0 = data_list.pt0M[blade_stage]  # ddd # 静叶进口总压，通流方案计算得出 asdfg
    Tt0 = data_list.Tt0M[blade_stage]  # ddd# 静叶进口总温，通流方案计算得出 asdfg
    Tt1 = Tt0  # 静叶出口总温，计算得来
    pt1 = pt0  # 静叶出口总压，计算得来
    # G = 87.5975  # 流量，给定
    # n = 5300  # 转速
    ...
    c1i = np.zeros(Zr)  # 静叶出口绝对速度 asdfg
    w1ui = np.zeros(Zr)  # 相对速度圆周分速
    w1i = np.zeros(Zr)  # 动叶进口相对速度 asdfg
    alpha1i = np.zeros(Zr)  # 绝对速度方向角，静叶出口气流角 asdfg
    Beta1i = np.zeros(Zr)  # 相对速度方向角，动叶进口相对气流角 asdfg
    a1i = math.sqrt(2 * data_list.k / (data_list.k + 1) * data_list.R * Tt1)  # 临界音速
    Lambda1i = np.zeros(Zr)  # 无因次速度（静叶出口马赫数） asdfg
    Ma_w1i = np.zeros(Zr)  # 动叶进口相对马赫数 asdfg
    T1i = np.zeros(Zr)  # 动叶进口静温（静叶出口静温） asdfg
    p1i = np.zeros(Zr)  # 动叶进口静压（静叶出口静压） asdfg
    ...
    w2i = np.zeros(Zr)  # 动叶出口相对速度 asdfg
    ...
    DT2i = h / cp  # 动叶中的总温差
    Tt2 = Tt1 - DT2i  # 气体总温
    T2i = np.zeros(Zr)  # 气体静温（动叶出口静温） asdfg
    ...
    RLambda2i = np.zeros(Zr)  # 出口相对无因次速度（动叶出口相对马赫数） asdfg
    Ideal_Rpt2i = np.zeros(Zr)  # 理论出口相对总压
    Co2i = np.zeros(Zr)  # 总压恢复系数
    Rpt2i = np.zeros(Zr)  # 出口相对总压
    p2i = np.zeros(Zr)  # 出口绝对静压（动叶出口静压） asdfg
    ...
    data_list.Lambda1i.append(Lambda1i)
    data_list.Ma_w1i.append(Ma_w1i)
    data_list.RLambda2i.append(RLambda2i)
    # data_list.Radial_Tt0.append(Tt0)
    # data_list.Radial_pt0.append(pt0)
