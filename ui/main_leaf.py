...
from ui import curve_set_window
...
class blade_designer(QtWidgets.QMainWindow, Ui_sCO2blade_designer):
    ...
    def __init__(self, opt_type, data=None, pp_init=None, ps_init=None):
        ...
        if self.type == 1:  # 1模式就没有data
            ...
            data_list.Rotor_t = [[0]]
            data_list.Rotor_z = [[0]]
        elif self.type == 2:  # 只要是一组叶片，不管是通过打开文件还是新建项目，都会有self.all_blade
            ...
            if pp_init is None:  # 新建一组叶片
                ...
                self.initUI()  # 设置malplotlib的连接
                self.initplot(pp_init[0][0][0], ps_init[0][0][0])
    ...
    def initplot(self, init_pp=None, init_ps=None):
    ...
    def slot_init(self):
        ...
        self.pushButton_about.clicked.connect(self.open_settings)
    ...
    def open_settings(self):
        if self.settings_window is None:
            self.settings_window = curve_set_window.SettingsWindow(self)  # 传递主窗口作为父窗口
        self.settings_window.show()

    def on_settings_changed(self, curve_type, param1, param2, param3=None, param4=None):
        if figure_operation.num_p != param1 or figure_operation.num_s != param2 \
                or figure_operation.curve_type != curve_type or figure_operation.pp_node_vector != param3 \
                or figure_operation.ps_node_vector != param4:
            figure_operation.num_p = param1
            figure_operation.num_s = param2
            figure_operation.pp_node_vector = copy.deepcopy(param3)
            figure_operation.ps_node_vector = copy.deepcopy(param4)
            figure_operation.curve_type = curve_type
            for i in range(data_list.Z_num):
                for j in range(data_list.Zr):
                    self.on_combo_change(i, j, 1)
            self.on_combo_change(0, 0, 0)
    ...
    def show_data(self):
        if self.type == 2 and self.current_zr < len(data_list.p2i[self.current_z]):
            ...
            self.data_window.lineEdit_pz_7.setText(f'{data_list.Dm1M[self.current_z] * 1000:.2f}')  # 静叶平均直径
            self.data_window.lineEdit_Tt0_8.setText(f'{data_list.alph0M[self.current_z]:.2f}')  # 静叶进口气流角
            self.data_window.lineEdit_pz_8.setText(f'{data_list.alph1M[self.current_z]:.2f}')  # 静叶出口气流角
            self.data_window.lineEdit_Maz_14.setText(f'{data_list.c1M[self.current_z]:.2f}')  # 静叶出口实际速度
            self.data_window.lineEdit_n_5.setText(f'{data_list.Lamda1M[self.current_z]:.2f}')  # 静叶出口马赫数
            ...
            self.data_window.lineEdit_alph2z_14.setText(f'{data_list.l1M[self.current_z] * 1000:.2f}')  # 静叶出口叶高

            self.data_window.lineEdit_Tt0_5.setText(f'{data_list.Dm2M[self.current_z] * 1000:.2f}')  # 动叶平均直径
            self.data_window.lineEdit_G_9.setText(f'{data_list.Beta1M[self.current_z]:.2f}')  # 动叶进口相对气流角
            self.data_window.lineEdit_pz_5.setText(f'{data_list.w1M[self.current_z]:.2f}')  # 动叶进口相对速度
            self.data_window.lineEdit_Tt0_6.setText(f'{data_list.Lamda11M[self.current_z]:.2f}')  # 动叶进口相对马赫数
            self.data_window.lineEdit_pz_6.setText(f'{data_list.Beta2M[self.current_z]:.2f}')  # 动叶出口相对气流角
            self.data_window.lineEdit_Efft0_4.setText(f'{data_list.w2M[self.current_z]:.2f}')  # 动叶出口相对速度
            self.data_window.lineEdit_Eff_13.setText(f'{data_list.Lamda12M[self.current_z]:.2f}')  # 动叶出口相对马赫数
            self.data_window.lineEdit_Maz_13.setText(f'{data_list.alph12M[self.current_z]:.2f}')  # 动叶出口绝对气流角
            self.data_window.lineEdit_n_4.setText(f'{data_list.l2M[self.current_z] * 1000:.2f}')  # 动叶出口叶高
            self.data_window.lineEdit_omaga_9.setText(f'{data_list.Bz1M[self.current_z] * 1000:.2f}')  # 静叶中径处叶宽
            self.data_window.lineEdit_xaz_9.setText(f'{data_list.Bz2M[self.current_z] * 1000:.2f}')  # 动叶中径处叶宽
            self.data_window.lineEdit_alph2z_13.setText(f'{data_list.ACM[self.current_z] * 1000:.2f}')  # 轴向间隙
            self.data_window.lineEdit_alph2z_15.setText(f'{data_list.LM[self.current_z] * 1000:.2f}')  # 通流部分长度
            ...
            self.data_window.lineEdit_G_12.setText(f'{data_list.Tt0M[self.current_z]:.2f}')  # 静叶进口总温
            self.data_window.lineEdit_Eff_19.setText(f'{data_list.pt0M[self.current_z] / 1000:.2f}')  # 静叶进口总压
            self.data_window.lineEdit_Eff_20.setText('90')  # 静叶进口气流角
            self.data_window.lineEdit_Eff_17.setText(
                f'{data_list.c1i[self.current_z][self.current_zr]:.2f}')  # 静叶出口绝对速度
            self.data_window.lineEdit_Maz_17.setText(
                f'{data_list.alpha1i[self.current_z][self.current_zr]:.2f}')  # 静叶出口气流角
            self.data_window.lineEdit_k_8.setText(
                f'{data_list.Lambda1i[self.current_z][self.current_zr]:.2f}')  # 静叶出口马赫数
            self.data_window.lineEdit_R_8.setText(f'{data_list.T1i[self.current_z][self.current_zr]:.2f}')  # 静叶出口静温
            self.data_window.lineEdit_omaga_12.setText(
                f'{data_list.p1i[self.current_z][self.current_zr] / 1000:.2f}')  # 静叶出口静压
            self.data_window.lineEdit_xaz_12.setText(
                f'{data_list.Beta1i[self.current_z][self.current_zr]:.2f}')  # 动叶进口相对气流角

            self.data_window.lineEdit_Eff_18.setText(
                f'{data_list.w1i[self.current_z][self.current_zr]:.2f}')  # 动叶进口相对速度
            self.data_window.lineEdit_faiz_8.setText(
                f'{data_list.Ma_w1i[self.current_z][self.current_zr]:.2f}')  # 动叶进口相对马赫数
            self.data_window.lineEdit_rouz_8.setText(
                f'{data_list.w2i[self.current_z][self.current_zr]:.2f}')  # 动叶出口相对速度
            self.data_window.lineEdit_taoz_8.setText(
                f'{data_list.Beta2i[self.current_z][self.current_zr]:.2f}')  # 动叶出口气流角
            self.data_window.lineEdit_fRz_z_8.setText(
                f'{data_list.RLambda2i[self.current_z][self.current_zr]:.4f}')  # 动叶出口相对马赫数
            self.data_window.lineEdit_Maz_18.setText(
                f'{data_list.T2i[self.current_z][self.current_zr]:.2f}')  # 动叶出口静温
            self.data_window.lineEdit_pt0_8.setText(
                f'{data_list.p2i[self.current_z][self.current_zr] / 1000:.2f}')  # 动叶出口静压
            ...
            self.data_window.lineEdit_alph2z_23.setText(f'{data_list.Rotor_z[self.current_z][0]}')  # 动叶叶片数
        ...
        self.data_window.lineEdit_alph2z_25.setText(f'{data_list.Stator_z[self.current_z][0]}')  # 静叶叶片数
    ...
    def feed_back_to_figure_and_camber(self, z, zr, opt):
        ...
        if figure_operation.curve_type == 1:
            figure_operation.re_p = calcu.blade_curve(figure_operation.pp, figure_operation.npoint - 1)
        else:
            figure_operation.re_p = calcu.blade_curve(figure_operation.pp, figure_operation.npoint - 1,
                                                      figure_operation.pp_node_vector)
        ...
        if figure_operation.curve_type == 1:
            figure_operation.re_s = calcu.blade_curve(figure_operation.ps, figure_operation.npoint - 1)
        else:
            figure_operation.re_s = calcu.blade_curve(figure_operation.ps, figure_operation.npoint - 1,
                                                      figure_operation.ps_node_vector)
        ...
        if self.ready_flag is True:  # 刚开始所有叶片过一遍不需要这些
            ...
            if z != self.last_z or opt != self.last_opt or self.type == 1:  # 改到其他叶片才需要重画3d
                ...
                hub_shroud_operation.shroud = calcu.blade_curve(hub_shroud_operation.pp,
                ...
                hub_shroud_operation.hub = calcu.blade_curve(hub_shroud_operation.ps, hub_shroud_operation.npoint - 1)
    ...
    def get_rep_and_res_from_cp(self, z, zr, opt):
        ...
        tmp_ps, waste = figure_operation.get_origin_control_point(figure_operation.num_s, True,
                                                                  ...
                                                                  self.all_blade[z][zr][opt][0])
        if figure_operation.curve_type == 1:
            tmp_re_p = calcu.blade_curve(tmp_pp, figure_operation.npoint - 1)
        else:
            tmp_re_p = calcu.blade_curve(tmp_pp, figure_operation.npoint - 1, figure_operation.pp_node_vector)
        xp = np.concatenate(([tmp_pp[0, 0]], tmp_re_p[:, 0]))
        yp = np.concatenate(([tmp_pp[0, 1]], tmp_re_p[:, 1]))
        tmp_re_p = np.column_stack((xp, yp))

        if figure_operation.curve_type == 1:
            tmp_re_s = calcu.blade_curve(tmp_ps, figure_operation.npoint - 1)
        else:
            tmp_re_s = calcu.blade_curve(tmp_ps, figure_operation.npoint - 1, figure_operation.ps_node_vector)
